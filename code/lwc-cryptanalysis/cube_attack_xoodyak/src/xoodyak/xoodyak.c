...
void xor_(uint8_t *output, uint8_t *block, uint8_t size);
...
void absorb_key(Xoodyak_data *xoodyak_data, uint8_t *id, uint8_t id_len, uint8_t *counter, uint32_t counter_len)
{
    ...
    uint8_t *buffer = (uint8_t*)malloc(KEY_SIZE + id_len + 1); 
...
void down(Xoodyak_data *xoodyak_data, uint8_t *data, uint32_t data_len, uint8_t color)
{
    ...
    xor_(xoodyak_data->state, data, data_len);
...
//    uint8_t tag[20];
//    uint8_t *decrypted_plaintext = (uint8_t*)malloc(message_len);
//
