...
void save_cube(const Cube& cube);
...
std::vector<Cube> random_walk()
{
    ...
    const Linearity_tester& linearity_tester = BLR_linearity_tester(cipher_text_cipher_output_finder);
...
void find_cubes_for_size(int cube_size)
{
    Cipher_text_cipher_output_finder cipher_text_cipher_output_finder{};
    Tag_cipher_output_finder tag_cipher_output_finder{};
    const Linearity_tester& linearity_tester = BLR_linearity_tester(cipher_text_cipher_output_finder);
    ...
    while (true)
    {
        Cube cube = Cube::random(cube_size);
        Linearity_tester::Linearity_test_result test_result = linearity_tester.is_linear(cube);   
        std::cout << "Cube test: " << cube << ", " << test_result << std::endl;
    
        if (test_result == Linearity_tester::linear)
	   save_cube(cube);
    }
}

std::vector<Cube> random_series()
{
    std::vector<Cube> cubes;
    std::vector<std::thread> threads;

    for (int cube_size = 12; cube_size < max_cube_size; cube_size++)
	threads.emplace_back(find_cubes_for_size, cube_size);
...
void save_to_file(std::vector<Cube> cubes)
{
    std::ofstream fs("cubes.txt");
    for (Cube& cube : cubes)
	fs << cube << std::endl;
...
void save_cube(const Cube& cube)
{
    std::ofstream fs(save_file, std::fstream::app);
    fs << cube << std::endl;;
    fs.close();
}
